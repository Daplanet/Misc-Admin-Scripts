		...
		foreach (@file) {
			...
			if ( $_ !~ m/^\s*#/ ) {
				...
				if ( $_ =~ m/\s*include\s+/i ) {
					...
					# by James Portman 19/06/2012 jamesportman@gmail.com
					# remove quotes from file names - needed for plesk 11 and apache files which do includes like:
					# Include '/bla/bla/file.conf'
                                        # Include "/bla/bla/file.conf"
					$_=~ s/\'//g;
                                        $_=~ s/\"//g;
